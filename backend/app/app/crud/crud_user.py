...
class CRUDUser(CRUDBase[User, CreatingUser, UpdatingUser]):
    ...
    def delete_user(self, db: Session, *, user: User):
        ...
        for user_subscription in user.subject_subscriptions:
            user_subscription.object.subscribers_count -= 1
            db.add(user_subscription.object)
        db.delete(user)
        db.commit()
    ...
    def subscribe(self, db: Session, *, subject: User, object_: User, new_value: bool):
        subscription = db.query(Subscription).filter(Subscription.object_ == object_, Subscription.subject == subject) \
            .first()
    ...
    def get_subscriptions(self, db: Session, *, user: User):
        return [s.object_ for s in user.subject_subscriptions if s.object_.deleted == None]

    def get_subscribers(self, db: Session, *, user: User):
